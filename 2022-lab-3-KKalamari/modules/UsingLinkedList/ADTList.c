...
void list_remove(List list, ListNode node) {
	for(ListNode prev_node = list_first(list);     
    prev_node != LIST_EOF;                          
    prev_node = list_next(list, prev_node)) {            
...
ListNode prev_node(List list,ListNode node) {
	for(ListNode prev_node = list_first(list);     
    prev_node != LIST_EOF;                          
    prev_node = list_next(list, prev_node)) {            
...
void list_append(List list, List to_append) {
	...
	for(ListNode node= list_last(list);
		node!=LIST_EOF;
		node= list_next(list,node)) {
			for(int i=1; i<to_append->size;i++)
