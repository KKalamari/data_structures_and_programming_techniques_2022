...
static void good_heapify(PriorityQueue pqueue, Vector values) {
	//παω τα στοιχεια του values στο pqueue->vector
	for(int x=0; x<vector_size(values); x++) 
		vector_insert_last(pqueue->vector, vector_get_at(values,x));

	for(int x=vector_size(pqueue->vector)/2; x>=0; x--) 
		bubble_down(pqueue,x);
}
