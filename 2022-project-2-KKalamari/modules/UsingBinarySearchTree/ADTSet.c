...
SetNode for_tree(Vector vec, int first, int for_last) {

	if(first > for_last)	
		return NULL;
	int the_root = (first + for_last)/2;
	SetNode root = vector_get_at(vec, the_root);
	if(root->left ==root || root->right == root) 
		return NULL;
	root->left = for_tree(vec,first,the_root-1);


	root->right = for_tree(vec,the_root+1,for_last);
	return root;
}
...
Set set_create_from_sorted_values(CompareFunc compare, DestroyFunc destroy_value, Vector values) {
	...
	for(int i = 0; i<size; i++) {
		set_insert(set,vector_get_at(values,i));
	}
	
	set->root = for_tree(values,0,size-1);
