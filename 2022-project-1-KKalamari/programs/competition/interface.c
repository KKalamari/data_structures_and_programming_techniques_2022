...
void interface_init() {
	...
	hakuu = LoadSound("assets!/haku!!");
    spirited_away = LoadSound("assets!/spirited_away_theme.mp3");
}

void interface_close() {
	UnloadTexture(faceless_manleft);
	UnloadTexture(haku);
	...
	UnloadTexture(ubabaleft);
	UnloadTexture(Background);
	UnloadSound(spirited_away);
	UnloadSound(hakuu);
	//UnloadImage(Backround);
    CloseAudioDevice();
    CloseWindow();
}
...
void interface_draw_frame(State state) {
	...
	//ImageClearBackground(&Backround,WHITE);
	...
	for(ListNode node = list_first(items); node != LIST_EOF; node = list_next(items,node)) {
		...
		if(item->type== BRIDGE)
			DrawTexture(lanters,0,item->rect.y+((state_info(state)->jet->rect.y) * -1) +275,WHITE);
	...
	if(frames == 1)
		PlaySound(spirited_away);

	if(state_info(state)->game_over) {
		StopSound(spirited_away);
		frames = 0;
	}
		// PlaySound(game_over);
	...
	if(state_info(state)->paused)
		PauseSound(spirited_away);
	else
		ResumeSound(spirited_away);
